...
const MusicPlayer = React.forwardRef<MusicPlayerHandle, MusicPlayerProps>(
  ({ className = '', outline, gameEndsAt }, ref) => {
    ...
    React.useEffect(() => {
      if (!gameEndsAt || !sound) return

      const fadeInterval = setInterval(() => {
        const now = Math.floor(Date.now() / 1000)
        const timeRemaining = gameEndsAt - now

        // Start fading out in the last 10 seconds
        if (timeRemaining <= 10 && timeRemaining > 0) {
          const fadeVolume = (timeRemaining / 10) * targetVolume.current
